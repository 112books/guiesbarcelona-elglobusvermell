...
- ✅ **Accessibilitat restant — FETA (10 set)**: Leaflet teclat ✅ (2 set), aria-required + errors ✅ (2 set), «(PDF)» ✅ (2 set), pa11y CI ✅ (10 set) — bug: hugo server sense --environment staging servia sense subpath, pa11y donava 404; fix + 4 correccions contrast CSS (filtre-grup-label, llistat-grup-count, fitxa-pub-tag, publicacio-cap h1 via color-mix); 10/10 URLs passa
...
- ✅ **Autoria del codi (1) — identitat git FETA el 2 set**: els 179 commits «El teu nom <el-teu-email@example.com>» reescrits (author + committer) a **LinuxBCN <hola@linuxbcn.com>** (decisió de Joan 2 set) via `git filter-branch`; contingut dels commits intacte (el `tree` dels fitxers no ha canviat); força-push a `main` + tag `v0.1-mapes-i-paper`; backup del repo pre-canvi a `../guiesbcn-backup-autoria.git`. Queden pendents: (2) **llicència del codi** — el contingut és CC BY-SA 4.0 però el codi del tema i els scripts no en tenen cap; (3) crèdits d'autoria al web i al repo
